---
name: mymithra
description: Generate the integration pipeline scaffold (Provider/Validator/Converter/Publisher + Logger + IntegrationPipeline) on top of a Sintaura-scaffolded Java project. Activate when the user asks to scaffold/genera pipeline, integration, orchestratore, run-time wiring for a Dataverse entity that already has Sintaura models + Provider, or wants to add validation/conversion/publish steps to a Sintaura project.
---

# Mymithra — Dataverse integration pipeline scaffold

Mymithra completes the trio **Grimoire (docs) → Sintaura (stub/test) → Mymithra (operational pipeline)**. Given a Java project already scaffolded by Sintaura (with `models/{Entity}.java` + `providers/{Entity}Provider.java`), Mymithra generates:
...
When the source endpoint uses `$expand` (the Sintaura DTO has nested entity fields), Mymithra walks the tree recursively and generates a parent + child scaffold for each level. Composition is direct: the parent component receives the child as a constructor dependency.
...
- The `pom.xml` must include Mockito (`mockito-core` + `mockito-junit-jupiter`, version 5.11+). Sintaura adds these by default since v3; older Sintaura projects need a manual update.
- The `{Entity}Provider` class must implement `Provider<{Entity}>` from `{base}.core.Provider`. Sintaura templates v3 emit this automatically. If missing, instruct the user to regenerate via Sintaura.

## Inputs

The user provides, via `AskUserQuestion`:

1. **Output directory** — absolute path of the existing Sintaura project root (default: detect from current working directory).
2. **Root entity** — the entity to orchestrate (e.g. `Appointment`). Mymithra rejects entity names that have no matching `models/{Entity}.java`.
3. **Java package base** — default: detect from the Sintaura project (e.g. `com.acme.dataverse`).
...
If `{base}/core/Provider.java` exists, **skip** the entire core generation (Mymithra has already run, or Sintaura v3 has provided `Provider.java`). Otherwise, copy the 17 templates from `templates/core/` to `src/main/java/{base}/core/`, substituting `{{PACKAGE_BASE}}` with the user's package base. Same for the 4 templates in `templates/core/test/` → `src/test/java/{base}/core/`.
...
For child entities (no nested `$expand`), `{Child}ValidatorTest`/`ConverterTest`/`PublisherTest` are simpler: one or two tests verifying the scaffold default behavior (`ok()`, returns non-null `{Child}Output`, logs `publish: …`).
...
- `templates/entity/*.java.template` — 5 files: `EntityOutput`, `EntityValidator`, `EntityConverter`, `EntityPublisher`, `EntityIntegration`. Single-child reference; the skill replicates the per-child blocks for N children.
...
| `{{CHILD_NAV_PROPERTY_UPPER}}` | upper-snake-case of the nav property used as constant name (e.g. `REGARDINGOBJECTID_ACCOUNT_APPOINTMENT`) |
| `{{CHILD_GETTER}}` | parent-side getter (e.g. `getRegardingObjectIdAccountAppointment`) |
| `{{CHILD_SETTER}}` | parent-side setter (e.g. `setRegardingObjectIdAccountAppointment`) |
| `{{CHILD_VAR}}` | camelCase prefix of the child (e.g. `account`); used to derive `accountValidator`, `accountConverter`, `accountPublisher`, `accountOutput` |

When N > 1 child, the skill repeats the per-child block N times in the parent (constructor parameter, field, assignment, `if (in.get{Child}() != null) …` invocation in `doX()`, child instantiation in `buildPipeline()`).
